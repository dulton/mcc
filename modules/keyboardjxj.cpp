...
class QueryThread : public ThreadIf {
	protected:
		...
		QueryThread(SerialPort &keyboard) : ThreadIf("QueryThread"), mKeyboard(keyboard) {}
	protected:
		virtual void thread()
		{
			char query = 0x50;
...
{
	public:
		KeyboardJXJ(HandleManager& mgr, Postable *receiver, ModuleInfo *info) : SerialKeyboard(mgr, receiver, info) , mThread(NULL) {}
		virtual const char* name() const { return "KeyboardJXJ";}
